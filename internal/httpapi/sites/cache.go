...
func getCache(db *client.Client) echo.HandlerFunc {
	return func(c *echo.Context) error {
		...
		if site.PolicyId != nil {
			stored, findErr := db.Policy.FindUnique(c.Request().Context(), query.Policy.Id.Equals(*site.PolicyId))
			if findErr != nil {
				return findErr
			}
...
func updateCache(db *client.Client, publishService *publisher.Service) echo.HandlerFunc {
	return func(c *echo.Context) error {
		...
		err = db.Tx(ctx, func(tx *client.Client) error {
			if site.PolicyId != nil {
				_, updateErr := tx.Policy.Update().Where(query.Policy.Id.Equals(*site.PolicyId)).Set(query.Policy.CacheJson.Set(encoded)).Do(ctx)
				return updateErr
			}
			policyID := uuid.NewString()
			empty := json.RawMessage(`{}`)
			if _, createErr := tx.Policy.Create().Set(query.Policy.Id.Set(policyID), query.Policy.Name.Set("site:"+site.Id), query.Policy.CacheJson.Set(encoded), query.Policy.WafJson.Set(empty), query.Policy.CcJson.Set(empty), query.Policy.AccessJson.Set(empty)).Do(ctx); createErr != nil {
				return createErr
			}
