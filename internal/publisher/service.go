...
func (s *Service) Enqueue(ctx context.Context, siteID string) (*model.PublishJob, error) {
	...
	err := s.db.Tx(ctx, func(tx *client.Client) error {
		...
		job, err = tx.PublishJob.Create().Set(query.PublishJob.SiteId.Set(siteID), query.PublishJob.Version.Set(version), query.PublishJob.Targets.Set(targetJSON), query.PublishJob.Status.Set(model.JobStatusPENDING)).Do(ctx)
		return err
	})
	return job, err
}
...
func (s *Service) execute(ctx context.Context, job *model.PublishJob) error {
	...
	version, err := s.db.ConfigVersion.Query().Where(query.ConfigVersion.SiteId.Equals(job.SiteId), query.ConfigVersion.Version.Equals(job.Version)).First(ctx)
	if err != nil {
		return s.finish(ctx, job.Id, model.JobStatusFAILED, nil, err)
	}
	...
	if len(failed) == 0 {
		if _, err = s.db.Site.Update().Where(query.Site.Id.Equals(site.Id)).Set(query.Site.Version.Set(job.Version)).Do(ctx); err != nil {
			return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
		}
		if _, err = s.db.ConfigVersion.Update().Where(query.ConfigVersion.SiteId.Equals(site.Id), query.ConfigVersion.Version.Equals(job.Version)).Set(query.ConfigVersion.Status.Set(model.ConfigStatusPUBLISHED)).DoMany(ctx); err != nil {
			return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
		}
		for _, item := range succeeded {
			if err = s.setNodeVersion(ctx, item.NodeID, site.Id, job.Version, model.ConfigStatusPUBLISHED); err != nil {
				return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
			}
		}
	...
	rollback := edgeprotocol.SiteConfig{SiteID: site.Id, Version: uint64(rollbackVersion), Disabled: true}
	previous, previousErr := s.db.ConfigVersion.Query().Where(query.ConfigVersion.SiteId.Equals(site.Id), query.ConfigVersion.Status.Equals(model.ConfigStatusPUBLISHED)).OrderBy(query.ConfigVersion.Version.Desc()).First(ctx)
	if previousErr == nil {
	...
	if _, err = s.db.ConfigVersion.Update().Where(query.ConfigVersion.SiteId.Equals(site.Id), query.ConfigVersion.Version.Equals(job.Version)).Set(query.ConfigVersion.Status.Set(model.ConfigStatusFAILED)).DoMany(ctx); err != nil {
		return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
	}
	...
	if _, err = s.db.ConfigVersion.Create().Set(query.ConfigVersion.SiteId.Set(site.Id), query.ConfigVersion.Version.Set(rollbackVersion), query.ConfigVersion.ConfigJson.Set(rollbackJSON), query.ConfigVersion.Hash.Set(hex.EncodeToString(rollbackHash[:])), query.ConfigVersion.Status.Set(rollbackStatus)).Do(ctx); err != nil {
		return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
	}
	if rollbackComplete {
		if _, err = s.db.Site.Update().Where(query.Site.Id.Equals(site.Id)).Set(query.Site.Version.Set(rollbackVersion)).Do(ctx); err != nil {
			return s.finish(ctx, job.Id, model.JobStatusFAILED, results, err)
		}
	}
	for _, rolled := range rollbackResults {
		if rolled.Success {
			_ = s.setNodeVersion(ctx, rolled.NodeID, site.Id, rollbackVersion, model.ConfigStatusROLLED_BACK)
		}
	}
...
func (s *Service) fanout(ctx context.Context, config edgeprotocol.SiteConfig, targets []target) []targetResult {
	...
	for index, item := range targets {
		...
		go func() {
			...
			_, err := edgecontrol.New(endpoint, item.NodeID, item.CommunicationKey).PushSiteConfig(ctx, config)
...
func (s *Service) finish(ctx context.Context, jobID string, status model.JobStatus, results any, executionErr error) error {
	...
	_, err := s.db.PublishJob.Update().Where(query.PublishJob.Id.Equals(jobID)).Set(query.PublishJob.Status.Set(status), query.PublishJob.ResultJson.Set(payload)).Do(ctx)
	if err != nil {
		return err
	}
	return executionErr
...
func (s *Service) buildWith(db *client.Client, ctx context.Context, site *model.Site, version uint64) (edgeprotocol.SiteConfig, []target, error) {
	domains, err := db.SiteDomain.Query().Where(query.SiteDomain.SiteId.Equals(site.Id)).Do(ctx)
	if err != nil {
	...
	backends, err := db.OriginBackend.Query().Where(query.OriginBackend.OriginPoolId.Equals(pool.Id), query.OriginBackend.Enabled.Equals(true)).Do(ctx)
	if err != nil {
		return edgeprotocol.SiteConfig{}, nil, err
	}
	config := edgeprotocol.SiteConfig{SiteID: site.Id, Version: version, Scheduler: pool.Scheduler, Listener: edgeprotocol.ListenerConfig{HTTPEnabled: listener.HttpEnabled, HTTPPort: listener.HttpPort, RedirectHTTPToHTTPS: listener.RedirectHttpToHttps, HTTPSEnabled: listener.HttpsEnabled, HTTPSPort: listener.HttpsPort, HTTP2Enabled: listener.Http2Enabled, HTTP3Enabled: listener.Http3Enabled, TLSMinVersion: string(listener.TlsMinVersion), HSTSEnabled: listener.HstsEnabled, HSTSMaxAge: listener.HstsMaxAge, HSTSIncludeSubdomains: listener.HstsIncludeSubdomains, HSTSPreload: listener.HstsPreload, OCSPStaplingEnabled: listener.OcspStaplingEnabled}}
	for _, item := range domains {
		config.Domains = append(config.Domains, item.Hostname)
	}
	for _, item := range backends {
		config.Origins = append(config.Origins, edgeprotocol.OriginConfig{Protocol: strings.ToLower(string(item.Protocol)), Address: item.Address, HostHeader: value(item.HostHeader), Weight: item.Weight})
	}
	links, err := db.SiteCertificate.Query().Where(query.SiteCertificate.SiteId.Equals(site.Id)).Do(ctx)
	if err != nil {
	...
	for _, link := range links {
		...
		config.Certificates = append(config.Certificates, edgeprotocol.CertificateConfig{CertificatePEM: certificate.CertPem, PrivateKeyPEM: certificate.PrivateKeyPem})
	}
	...
	nodes, err := db.Node.Query().Where(query.Node.ClusterId.Equals(site.ClusterId)).Do(ctx)
	if err != nil {
		return config, nil, err
	}
	...
	for _, n := range nodes {
		...
		addresses, addressErr := db.NodeAddress.Query().Where(query.NodeAddress.NodeId.Equals(n.Id), query.NodeAddress.Primary.Equals(true)).First(ctx)
...
func (s *Service) loadCredentials(ctx context.Context, targets []target) error {
	for i := range targets {
		credential, err := s.db.NodeCredential.FindUnique(ctx, query.NodeCredential.NodeId.Equals(targets[i].NodeID))
		if err != nil {
